/* terra:network */
export const FINDER = 'https://finder.terra.money';
export const EXTENSION = 'https://terra.money/extension';
export const CHROME = 'https://google.com/chrome';
export const DOCS = 'https://docs.astroportprotocol.com';
export const ICON_URL = 'https://whitelist.mirror.finance/images';

/* astroport:unit */
export const SMALLEST = 1e6;

export const ONE_TOKEN = 1000000;

export const GRAPHQL = process.env.NEXT_PUBLIC_GRAPHQL_ENDPOINT;

/* New fund constants */
export const DP_CODE_ID = 1572;
export const FEE_AMOUNT = '0.05';
export const FEE_COLLECTOR = 'terra1yt8xwehw3mngc7h2z09utf32cgpdpe09lke902';
export const FEE_MAX = '0';
export const FEE_RESET_EVERY_NUM_BLOCKS = 0;
export const GO_FUND_UST_CODE_ID = 42147;
export const TEST_POOL = 'terra1rkr7p4ad7sn602xy8mk7guukwkqtq2ncag0pde';
